/**
 * React hook that ties together the Base44 identity system:
...
export function useBase44Identity() {
  ...
  // Derive creator + autosplit identities whenever the wallet connects.
  useEffect(() => {
    if (!walletAddress) {
      setCreator(null);
      setAutosplit(null);
  ...
  const base44Login = useCallback(async () => {
    ...
    try {
      ...
      return { res, wallet, packet, creator: ci, autosplit: ai };
    } catch (e) {
